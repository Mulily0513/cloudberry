...
SELECT MAX(w.watermark) AS wm_after_partial
FROM time_series.cagg_watermark w
JOIN time_series.continuous_agg c ON w.cagg_id = c.cagg_id
WHERE c.user_view_name = 'cv';
...
-- ============================================================
-- R-04: Non-aligned window → ERROR (matches TimescaleDB)
--       [01:25, 02:45) with 1h buckets → inscribed [02:00, 02:00) = empty!
--       TimescaleDB reports: "refresh window too small"
-- ============================================================
...
CREATE TABLE src_origin (time TIMESTAMPTZ NOT NULL, dev INT, val INT)
  DISTRIBUTED BY (dev);
...
CREATE TABLE src_offset (time TIMESTAMPTZ NOT NULL, dev INT, val INT)
  DISTRIBUTED BY (dev);
...
CREATE TABLE src_tz (time TIMESTAMPTZ NOT NULL, dev INT, val INT)
  DISTRIBUTED BY (dev);
INSERT INTO src_tz
...
CREATE TABLE empty_src (time timestamptz NOT NULL, tags_id int NOT NULL, val float8)
  DISTRIBUTED BY (tags_id);
...
CREATE TABLE metrics_1s (time TIMESTAMPTZ NOT NULL, v INT) DISTRIBUTED BY (v);
...
CREATE TABLE metrics_dist (time TIMESTAMPTZ NOT NULL, tags_id INT NOT NULL, val FLOAT8)
  DISTRIBUTED BY (tags_id);
...
CREATE TABLE metrics_filt (time TIMESTAMPTZ NOT NULL, v INT NOT NULL, val FLOAT8)
  DISTRIBUTED BY (v);
...
CREATE TABLE ns_src (time timestamptz NOT NULL, tags_id int NOT NULL, val float8)
  DISTRIBUTED BY (tags_id);
...
-- ============================================================
-- TSDB-aligned tests: P1 + P2 gaps
-- ============================================================

-- ============================================================
-- P1-DATE: DATE type CAGG REFRESH (TSDB REF-31~32)
...
-- ============================================================
-- P1-NONCAGG: REFRESH on non-CAGG object → error (TSDB REF-20)
-- ============================================================
...
-- ============================================================
-- P2-ALIGN: Inscribed bucketing (TSDB REF-06~09)
-- TSDB rejects non-aligned; we inscribe (only complete buckets).
-- ============================================================
...
-- ============================================================
-- P2-STDREFRESH: Standard REFRESH MATERIALIZED VIEW (TSDB REF-41)
-- ============================================================
...
-- ============================================================
-- P2-TXNCTX: Transaction context restrictions (TSDB REF-48~51)
-- ============================================================
...
-- FUNCTION context → should error, not crash (TSDB REF-49)
\set ON_ERROR_STOP 0
...
-- EXCEPTION block context → should error, not crash (TSDB REF-48)
\set ON_ERROR_STOP 0
DO $$
BEGIN
  BEGIN
...
-- Explicit transaction block → should error (TSDB REF-42~43)
...
-- Trigger context → should error (TSDB cagg_refresh_common.sql line 395)
...
-- ============================================================
-- P2-OFFSET-REFRESH: REFRESH on origin CAGG (TSDB QRY-35)
-- ============================================================
...
-- ============================================================
-- P2-WITHDATA: CREATE defaults to no auto-populate (TSDB REF-40)
-- ============================================================
...
-- ============================================================
-- P3 tests: remaining TSDB gaps
-- ============================================================

-- ============================================================
-- P3-TZ: PDT timezone window (TSDB REF-05)
-- ============================================================
...
-- ============================================================
-- P3-BADARGS: Invalid argument types/formats (TSDB REF-21~28)
...
-- Unparseable time string (TSDB cagg_refresh_common.sql line 105)
CALL time_series.refresh_continuous_aggregate('cv', 'xyz', NULL);
\set ON_ERROR_STOP 1

-- ============================================================
-- P3-HASHAGG: hashagg toggle doesn't affect result (TSDB QRY-13)
-- ============================================================
...
-- ============================================================
-- P3-PROCEDURE: REFRESH inside a custom PROCEDURE (TSDB REF-47)
-- ============================================================
...
-- ============================================================
-- P3-CATALOG: Corrupt bucket_func in catalog (TSDB QRY-30)
...
--            (TSDB cagg_refresh_common.sql: force=>true scenario)
--
--   Same limitation as TSDB: after manual DELETE of mat rows,
...
--
--   TSDB workaround: force=>true parameter.
...
-- Same behavior as TSDB without force=>true.
CALL time_series.refresh_continuous_aggregate('cv', NULL, NULL);
...
CREATE TABLE hot_src (time TIMESTAMPTZ NOT NULL, v INT) DISTRIBUTED BY (v);
