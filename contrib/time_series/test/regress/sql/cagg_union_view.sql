...
--       (named after TimescaleDB's cagg_union_view test for consistency)
-- ============================================================
...
SELECT cnt   = 10 AS rt04_cnt_ok,
  ...
  FROM cv
 WHERE bucket = '2024-01-01 05:00+00' AND tags_id = 1;
-- cv must be a perfect mirror of _mat_cv_1
...
CREATE TABLE metrics_fresh (time TIMESTAMPTZ NOT NULL, v INT NOT NULL, val FLOAT8)
  DISTRIBUTED BY (v);
...
-- ============================================================
-- RT-10: After full REFRESH, real-time branch contributes 0 rows
--        (everything materialized, nothing beyond watermark)
-- ============================================================
\echo '=== RT-10: full REFRESH → all from mat branch ==='
CALL time_series.refresh_continuous_aggregate('cv', NULL, NULL);
-- All rows served from mat table; count from user view equals count from mat
SELECT count(*) AS cv_total FROM cv;
SELECT count(*) AS mat_total FROM time_series._mat_cv_1;
SELECT count(*) AS diff_rt10 FROM (
  (  SELECT bucket, tags_id, cnt, avg_temp FROM cv
   EXCEPT
   SELECT bucket, tags_id, cnt, avg_temp FROM time_series._mat_cv_1)
  UNION ALL
...
CREATE TABLE metrics_empty (time TIMESTAMPTZ NOT NULL, v INT NOT NULL, val FLOAT8)
  DISTRIBUTED BY (v);
...
CREATE TABLE metrics_alias (time TIMESTAMPTZ NOT NULL, v INT NOT NULL, val FLOAT8)
  DISTRIBUTED BY (v);
...
CREATE TABLE metrics_hav (time TIMESTAMPTZ NOT NULL, v INT NOT NULL, val FLOAT8)
  DISTRIBUTED BY (v);
...
-- ============================================================
-- RT-18b: cagg_watermark() must NOT be constant-folded by planner
--
-- If the planner folds cagg_watermark(N) to a constant on QD, all
-- segments would use the same watermark value instead of reading their
-- own local row.  This test verifies that EXPLAIN shows the function
-- call (not a timestamp literal) in the plan's Filter.
--
-- Historical context: the function was originally LANGUAGE SQL STABLE,
-- which PG inlined + constant-folded.  Fixed by changing to LANGUAGE
-- plpgsql VOLATILE — VOLATILE prevents eval_const_expressions from
-- folding it at plan time.
-- ============================================================
\echo '=== RT-18b: cagg_watermark not constant-folded ==='
...
BEGIN
    ...
    -- Expected: funcexpr=true, const_ts=false
END $$;
...
CREATE TABLE m_plan (time TIMESTAMPTZ NOT NULL, v INT NOT NULL, val FLOAT8)
  DISTRIBUTED BY (v);
...
-- ============================================================
-- TSDB alignment: query/view tests
-- ============================================================

-- ============================================================
-- RT-24: Complex WHERE + HAVING + expressions in union view
--        (TSDB cagg_union_view.sql.in: sum(b+c), max-min, HAVING)
-- ============================================================
\echo '=== RT-24: complex WHERE+HAVING ==='
CREATE TABLE wh_src (time TIMESTAMPTZ NOT NULL, loc TEXT, b INT, c INT)
  DISTRIBUTED BY (loc);
...
--        (TSDB cagg_union_view.sql.in: timestamp_table)
-- ============================================================
\echo '=== RT-25: TIMESTAMP no tz ==='
CREATE TABLE ts_src (time TIMESTAMP NOT NULL, v INT NOT NULL, val FLOAT8)
  DISTRIBUTED BY (v);
...
--        (TSDB cagg_union_view.sql.in: error handling)
--        Aligned with TSDB: fail-fast on missing watermark.
-- ============================================================
\echo '=== RT-26: watermark invalid input ==='
-- Non-existing ID on QD → returns -infinity (QD has no RANDOMLY rows)
-- On segment QE, this would ERROR (fail-fast, same as TSDB).
...
--        (TSDB cagg_usage-17.sql: timezone handling)
-- ============================================================
\echo '=== RT-27: AT TIME ZONE ==='
CREATE TABLE tz_src (time TIMESTAMPTZ NOT NULL, v INT NOT NULL, temp FLOAT8)
  DISTRIBUTED BY (v);
...
--        (TSDB cagg_usage-17.sql: 3-arg timezone form)
-- ============================================================
\echo '=== RT-28: time_bucket timezone param ==='
CREATE TABLE tbtz_src (time TIMESTAMPTZ NOT NULL, v INT NOT NULL, val FLOAT8)
  DISTRIBUTED BY (v);
...
--        (TSDB cagg_query_common.sql: compare CAGG output
...
--        (TSDB cagg_usage-17.sql: observe old value before refresh,
...
ALTER VIEW cv SET (time_series.materialized_only = true);
SELECT cnt AS cnt_before_backfill FROM cv
WHERE bucket = '2024-01-01 01:00+00' AND tags_id = 1;

-- Backfill: add a row to same bucket
INSERT INTO metrics VALUES ('2024-01-01 01:45+00', 1, 99.0);

-- mat-only still shows OLD value (stale)
SELECT cnt AS cnt_stale FROM cv
WHERE bucket = '2024-01-01 01:00+00' AND tags_id = 1;

-- REFRESH then verify NEW value
ALTER VIEW cv SET (time_series.materialized_only = false);
CALL time_series.refresh_continuous_aggregate('cv',
  '2024-01-01 01:00+00', '2024-01-01 02:00+00');
ALTER VIEW cv SET (time_series.materialized_only = true);
SELECT cnt AS cnt_fresh FROM cv
WHERE bucket = '2024-01-01 01:00+00' AND tags_id = 1;
ALTER VIEW cv SET (time_series.materialized_only = false);

-- ============================================================
-- RT-32: ::timestamp cast in CAGG definition → rejected
--        (TSDB pre-PG17 behavior: reject STABLE expressions)
--
...
CREATE TABLE tz_cast (time TIMESTAMPTZ NOT NULL, device INT NOT NULL, temp FLOAT8)
...
--        (TSDB cagg_union_view.sql.in lines 468-528)
--
--   TSDB behavior: DELETE watermark → query ERROR, REFRESH ERROR.
...
-- Query should ERROR: watermark not found (fail-fast, same as TSDB)
\set ON_ERROR_STOP 0
