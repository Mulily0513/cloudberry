...
-- Clean-room implementation — not derived from TimescaleDB tests.
--
...
CREATE TABLE gf_ts (ts TIMESTAMP NOT NULL, grp INT NOT NULL) DISTRIBUTED BY (grp);
...
-- 1-year gapfill
CREATE TABLE gf_year (ts TIMESTAMPTZ NOT NULL, grp INT NOT NULL, value FLOAT8) DISTRIBUTED BY (grp);
...
CREATE TABLE gf_start0_tstz (ts TIMESTAMPTZ NOT NULL, grp INT NOT NULL) DISTRIBUTED BY (grp);
...
CREATE TABLE gf_month_tz2 (ts TIMESTAMPTZ NOT NULL, grp INT NOT NULL, value FLOAT8) DISTRIBUTED BY (grp);
...
CREATE TABLE gf_mpp_result (bucket TIMESTAMPTZ, avg_value FLOAT8)
    DISTRIBUTED BY (bucket);
...
-- advance_month_usec() is custom code (not from TimescaleDB),
