#!/bin/bash
# run_scale_tests.sh
# Orchestrate execution of all scale tests (concurrency / soak / repeat /
# schema_evolution / lightweight_recovery / catalog_consistency).
#
# Each category is a subdirectory under sqlrepo/scale/ with its own
...
log_info "==================================================================="
log_info "Running Datalake FDW Scale Tests"
log_info "==================================================================="
...
# Report directory
STAB_REPORT_DIR="${REPORTS_DIR}/scale/${REPORT_TIMESTAMP}"
...
run_scale_category() {
    local category=$1
    local test_dir="${SQLREPO_DIR}/scale/${category}"

    if [ ! -d "${test_dir}" ]; then
        log_warn "Scale test directory not found: ${test_dir}"
        return 0
    fi
    ...
    log_info "-------------------------------------------------------------------"
    log_info "Running ${category} scale tests..."
    log_info "-------------------------------------------------------------------"
    ...
    if make installcheck > "${STAB_REPORT_DIR}/${category}.log" 2>&1; then
        log_success "Scale test passed: ${category}"
        PASSED_TESTS=$((PASSED_TESTS + 1))
    else
        log_error "Scale test failed: ${category}"
...
log_info "Discovering scale test categories..."

# Categories to run - pass via SCALE_CATEGORIES env var to override
if [ -n "${SCALE_CATEGORIES:-}" ]; then
    STAB_CATEGORIES="${SCALE_CATEGORIES}"
    log_info "Using explicit SCALE_CATEGORIES='${STAB_CATEGORIES}'"
else
    STAB_CATEGORIES=$(find "${SQLREPO_DIR}/scale" -mindepth 1 -maxdepth 1 -type d \
                      -exec basename {} \; 2>/dev/null | sort)
fi

if [ -z "${STAB_CATEGORIES}" ]; then
    log_warn "No scale test categories found in ${SQLREPO_DIR}/scale"
    exit 0
fi

for category in ${STAB_CATEGORIES}; do
    run_scale_category "${category}"
done
