/*-------------------------------------------------------------------------
 *
 * nodeVecWindowHashAgg.c
 *	  routines to handle WindowHashAgg nodes.
 *
 * A WindowHashAgg node evaluates "window functions" across suitable partitions
 * of the input tuple set.  Any one WindowHashAgg works for just a single window
 ...
 * (The planner generates a stack of WindowHashAggs with intervening Sort nodes
 ...
 *
 * IDENTIFICATION
 *	  src/backend/vecexecutor/nodeVecWindowHashAgg.c
 *
 *-------------------------------------------------------------------------
 */
...
{
	NodeTag		type;
	WindowHashAggState *winstate;	/* parent WindowHashAggState */
...
static TupleTableSlot *ExecVecWindowHashAgg(PlanState *pstate);
...
initialize_peragg(WindowHashAggState *winstate, WindowFunc *wfunc,
				  WindowStatePerAgg peraggstate)
{
	...
	/*
	 * ExecInitWindowHashAgg already checked permission to call aggregate function
 ...
 */
static void
release_partition(WindowHashAggState *winstate)
{
	int			i;
...
WindowHashAggState *
ExecInitVecWindowHashAgg(WindowHashAgg *node, EState *estate, int eflags)
{
	VecWindowHashAggState *vec_winstate;
	WindowHashAggState *winstate;
	Plan	   *outerPlan;
	 ...
	 */
	vec_winstate = (VecWindowHashAggState *)palloc0(sizeof(VecWindowHashAggState));
	winstate = (WindowHashAggState *)vec_winstate;
	NodeSetTag(winstate, T_WindowAggState);

	winstate->ss.ps.plan = (Plan *) node;
	winstate->ss.ps.state = estate;
	winstate->ss.ps.ExecProcNode = ExecVecWindowHashAgg;
	...
	winstate->aggcontext =
		AllocSetContextCreate(CurrentMemoryContext,
                              "WindowHashAgg Aggregates",
                              ALLOCSET_DEFAULT_SIZES);

	/*
	 * WindowHashAgg nodes never have quals, since they can only occur at the
	...
	foreach(l, winstate->funcs)
	{
		...
		else
		{
			...
			winobj->winstate = (WindowHashAggState *)winstate;
	...
	if (winstate->numaggs > 0)
	{
		...
		agg_winobj->winstate = (WindowHashAggState *)winstate;
	...
	NodeSetTag(winstate, T_WindowHashAggState);
...
static void
ExecEagerFreeVecWindowHashAgg(VecWindowHashAggState *vnode)
{
...
{
	PlanState  	*outerPlan;
	VecWindowHashAggState *vnode;
	vnode = (VecWindowHashAggState *) node;
	/*
	...
	ExecEagerFreeVecWindowHashAgg(vnode);
...
ExecVecWindowHashAgg(PlanState *pstate)
{
	WindowHashAggState    *node  = castNode(WindowHashAggState, pstate);
	VecWindowHashAggState *vnode = (VecWindowHashAggState *) node;
